...
public:
    ...
    int query(int val, int start) {
        return query(start, val, 0, 0, size);
    }
// private:
    ...
    int query(int start, int val, int i, int l, int r) {
        if (r <= start) return -1;
        if (val > v[i].get()) return -1;
        if (r - l == 1) return l;
        int mid = l + (r - l)/2;
        int tmp = query(start, val, 2*i+1, l, mid);
        if (tmp != -1) return tmp;
        return query(start, val, 2*i+2, mid, r);
    }
};
